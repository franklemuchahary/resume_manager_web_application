...
/*
 *	This controller handles all the excel exports for the intern
...
class Intern_excel_exports extends CI_Controller{
	...
	public function intern_excel_received_applications()
	{
		...
		$exceldata="";
		foreach ($this->intern_recruiter_model->export_received_applications_excel() as $row){
			$exceldata[] = $row;
		}
        //Fill data 
        ...
        $filename= 'INTERN_'.date('d-m-y').'_ReceivedApplications_'.$session_data['username'].'.xls'; //save our workbook as this file name
    ...
    public function intern_excel_final_selected()
    {
    	$session_data = $this->session->userdata('intern_recruiter_is_logged_in');
        ...
        //retrive contries table data
    	$exceldata="";
    	foreach ($this->intern_recruiter_model->export_final_selected_excel() as $row){
    		$exceldata[] = $row;
    	}
        //Fill data 
    	$this->excel->getActiveSheet()->fromArray($exceldata, null, 'A5');
        ...
        $filename= 'INTERN'.date('d-m-y').'_FinalSelectedStudents_'.$session_data['username'].'.xls'; //save our workbook as this file name
        header('Content-Type: application/vnd.ms-excel'); //mime type
        header('Content-Disposition: attachment;filename="'.$filename.'"'); //tell browser what's the file name
        header('Cache-Control: max-age=0'); //no cache
