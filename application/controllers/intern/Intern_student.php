...
class Intern_student extends CI_controller{
	...
	public function __construct()
	{
		...
		elseif($this->session->userdata('intern_student_is_logged_in') && $this->_student_debarr_check() == true){
			redirect('intern/intern_student_debarred/', 'refresh');
		}
		else{
	...
	public function index()
	{
		...
		$config['base_url'] = base_url().'intern/intern_student/index/';
	...
	{
		$this->load->library('pagination');
		$config['base_url'] = base_url('intern/intern_student/job_openings/');
		...
		$data['info'] = $this->intern_student_model->student_navigation();
		$this->load->intern_student_view_template('intern/intern_student/jobopenings_view', $data);
	}
	//search job openings
	public function search_job_openings($search_string = '')
	{
		...
		$this->load->view('intern/intern_student/jobopenings_search_view', $data);
	}
//load the recruiter profile view when student clicks on a company to view it
	public function recruiter_profile($recruiter_username = 'hooli')
	{
		$data['specific_recruiter'] = $this->intern_student_model->get_specific_recruiter_info($recruiter_username);
		$data['student_info'] = $this->intern_student_model->get_personal_info();
		$data['final_selected'] = $this->intern_student_model->get_final_selected();

		if($this->intern_student_model->check_already_applied($recruiter_username)){
			$data['applied'] = "Your have successfully applied for this opening!";
		} 
		$data['info'] = $this->intern_student_model->student_navigation();
		$this->load->intern_student_view_template('intern/intern_student/recruiter_profile_view', $data);
	}
...
//first check for correct recruiter username apply for a company
	...
	{
		$check_recruiter_username = $this->intern_student_model->correct_recruiter_username($recruiter_username);
		$check_if_already_applied = $this->intern_student_model->check_already_applied($recruiter_username);
		$check_if_final_selected = $this->intern_student_model->get_final_selected();
		...
		if($check_recruiter_username && !$check_if_already_applied && ($check_if_debarred ==false) && ($check_if_final_selected ==false) && ($student_eligibility == true)){
			$query = $this->intern_student_model->enter_application($recruiter_username);
			if($query){
		...
		elseif($check_if_debarred == true){
			...
			redirect('intern/intern_student/recruiter_profile/'.$recruiter_username, 'refresh');
		}
		...
		elseif($student_eligibility == false){
			...
			redirect('intern/intern_student/job_openings', 'refresh');
		}
	}
	...
	{
		$specific_recruiter_result = $this->intern_student_model->get_specific_recruiter_info($recruiter_username);
		$student_info = $this->intern_student_model->get_personal_info();
...
//load my applications view
	...
	{
		$data['myapplications'] = $this->intern_student_model->get_applications_student();
		$data['info'] = $this->intern_student_model->student_navigation();
		$this->load->intern_student_view_template('intern/intern_student/myapplications_view', $data);
	}
...
//function to load profile personal info 
	...
	{
		$data['personal_info'] = $this->intern_student_model->get_personal_info();
		$data['info'] = $this->intern_student_model->student_navigation();
		$this->load->intern_student_view_parse_template('intern/intern_student/profile/personal_view', $data);
	}
//add and edit personal info into the database
	public function change_personal_info()
	{
		...
		if($this->form_validation->run() == FALSE){
			...
			$query = $this->intern_student_model->update_personal_info();
	...
	{
		$config['upload_path']          = '/var/www/html/git-repos/rm_dtu/uploads/intern_student_photos/';
		...
		else
		{
			...
			$query = $this->intern_student_model->upload_photo_path($file_name);
			if($query){
...
//function for loading academic profile view
	...
	{
		$data['academic_info'] = $this->intern_student_model->get_personal_info();
		$data['info'] = $this->intern_student_model->student_navigation();
		$this->load->intern_student_view_parse_template('intern/intern_student/profile/academic_view', $data);
	}
//edit and add academic info into the databse
	public function change_academic_info()
	{
		...
		if($this->form_validation->run() == FALSE){
			...
			$query = $this->intern_student_model->update_academic_info();
...
//load profile extras view
	...
	{
		$data['extra_info'] = $this->intern_student_model->get_personal_info();
		$data['info'] = $this->intern_student_model->student_navigation();
		$this->load->intern_student_view_parse_template('intern/intern_student/profile/extra_view', $data);
	}
//edit and add extra info into the database
	public function change_extra_info()
	{
		...
		if($this->form_validation->run() == FALSE){
			...
			$query = $this->intern_student_model->update_extra_info();
			$data['profile_updated'] = 'Your Profile has been Updated!';
			$data['extra_info'] = $this->intern_student_model->get_personal_info();
			$data['info'] = $this->intern_student_model->student_navigation();
			$this->load->intern_student_view_parse_template('intern/intern_student/profile/extra_view', $data);
	...
	{
		$data['info'] = $this->intern_student_model->student_navigation();
		$this->load->intern_student_view_template('intern/intern_student/profile/myaccount_view', $data);
	}
	...
	public function change_password()
	{
		...
		if($this->form_validation->run() == FALSE){
			...
			$query = $this->intern_student_model->change_student_password();
			if($query){
	...
	{
		$result = $this->intern_student_model->check_student_password($old_password);
		if($result){
			return true;
		}
		else{
	...
	public function change_security_question()
	{
		...
		if($this->form_validation->run()==FALSE){
			...
			$query = $this->intern_student_model->change_security_question();
			if($query){
