	...
	<section class="content">
		...
		<div class="row">
			...
			<div class="col-md-12">
				<div class="box">
					<div class="box-header with-border">
						<h3 class="box-title">Internship Description</h3>
					</div>
					<div class="box-body">
					...
					if(empty($applied) && empty($final_selected)){
						...
						//check for btech students
							if(!empty($btech) && !empty($student_branch_btech) && empty($student_branch_mtech)){
								...
								if($x == 1){
									if(!empty($student_btech_aggregate)&&$student_btech_aggregate>=$btech_cutoff){
										echo '<a href="'.base_url().'intern/intern_student/submit_application/'.$row['recruiter_username'].'"><button type="submit" class="update_button">Apply</button></a>';
									}
								}
							}
						//check for mtech students
