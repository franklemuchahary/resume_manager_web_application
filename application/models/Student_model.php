...
class Student_model extends CI_Model{
	...
	public function get_notifications($limit)
	{
		...
		$query = $this->db->get('notifications',$limit, $this->uri->segment(3));
		if($query){
			return $query->result_array();
		}
		else{
	...
	public function get_recruiter_info($limit, $uri_segment)
	{
		...
		$query = $this->db->get('recruiter',$limit, $this->uri->segment($uri_segment));
		if($query){
			return $query->result_array();
		}
		else{
	...
	public function enter_application($recruiter_username)
	{
		...
		if($query->num_rows() == 1){
			foreach($query->result_array() as $row){
				...
				$number_of_backlogs = $row['number_of_backlogs'];
				$backlog_subjects = $row['backlog_subjects'];
			}
		}
		//get recruiter information
		$this->db->where('recruiter_username', $recruiter_username);
		$query2 = $this->db->get('recruiter');
		...
		$data_to_insert = array(
			...
			$query3 = $this->db->insert('applications', $data_to_insert);
			if($query3){
				return true;
			}
			else{
	...
	public function update_academic_info()
	{
		$data_to_update = array(
				...
				'backlog_subjects' => $this->input->post('backlog_subjects'),
				'work_experience' => $this->input->post('work_experience'),
				'tnc_declaration' => $this->input->post('tnc_declaration')
	...
	{
		$data_to_update = array(
				'other_qualifications' => $this->input->post('other_qualifications', TRUE),
	...
	public function change_student_password()
	{
		...
		$this->db->where('username_rollnumber', $session_data['username']);
		$query = $this->db->update('student_personal', $data);
