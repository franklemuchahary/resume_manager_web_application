...
class Recruiter_model extends CI_Model{
	...
	public function insert_notification()
	{
		...
		$this->db->where('recruiter_username', $session_data['username']);
		$recruiter_query = $this->db->get('recruiter');
	...
	public function update_company_info()
	{
		...
		$session_data = $this->session->userdata('is_logged_in');
		$this->db->where('recruiter_username', $session_data['username']);
		$query = $this->db->update('recruiter', $data_to_insert);
		if($query){
			return true;
		}
		else{
			...
			foreach($this->input->post('student_rollnumber[]') as $key1=>$value1):
				foreach($this->input->post('status[]') as $key2=>$value2):
					if($key1 == $key2){
						...
						$query = $this->db->update('applications', $data_to_update);
					}
				endforeach;
			endforeach;
		else:
	...
	public function get_final_selected_applicants($limit)
	{
		...
		$query = $this->db->get('applications',$limit,$this->uri->segment(3));
		if($query){
			return $query->result_array();
		}
		else{
	...
	public function change_recruiter_password()
	{
		...
		$this->db->where('recruiter_username', $session_data['username']);
		$query = $this->db->update('recruiter', $data);
