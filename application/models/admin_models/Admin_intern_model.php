...
/**
* This is the intern admin model.
*
* This model will be responsible for all the database connections in the admin portal for intern. 
*/

class Admin_intern_model extends CI_Model {
	
	//method to check if the itnern admin or main admin is logged in
	public function intern_admin_logged()
	{
		$session_data = $this->session->userdata('admin_logged_in');
		$this->db->where('admin_username', $session_data['username']);
		$this->db->where('intern_admin', 1);
	...
	public function delete_notifications($id, $recruiter_username)
	{
		...
		$query = $this->db->delete('intern_notifications');
	...
	public function create_new_recruiter()
	{
		...
		$this->db->trans_start();
		$query = $this->db->insert('intern_recruiter', $data_to_insert);
	...
	public function undebarr_student_roll()
	{
		foreach($this->input->post('undebarr_roll_no[]') as $row):
			...
			$query = $this->db->update('intern_student', array('debarr' => NULL));
		endforeach;
	}
	...
	public function create_student_account()
	{
		...
		$query = $this->db->insert('intern_student', $data_to_insert);
	...
	public function get_student_info($limit)
	{
		...
		$query = $this->db->get('intern_student', $limit, $this->uri->segment(4));
		if($query){
			return $query->result_array();
		}
		else{
